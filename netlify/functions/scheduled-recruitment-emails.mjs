...
async function sendEmail({ to, subject, body }) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:680px;margin:auto;color:#172033;line-height:1.6">${String(body).split(/\n{2,}/).map((paragraph) => `<p>${escapeHtml(paragraph).replace(/\n/g, '<br>')}</p>`).join('')}</div>`
...
export default async () => {
  ...
  for (const document of familiesSnapshot.docs) {
    ...
    for (const cadet of family.cadets || []) {
      ...
      for (const candidate of candidates) {
        ...
        const body = `${fill(template.body, values)}\n\nReturn to the joining portal: ${returnLink}\n\nRegards,\n1330 Squadron Staff\n\nThis email address is not monitored. If you need assistance, please contact the Squadron at https://warringtonaircadets.com/.`
        const provider = await sendEmail({ to: family.guardian.email, subject, body })
        const message = { id: `message-${crypto.randomUUID()}`, familyId: family.id, cadetId: cadet.id, templateId: candidate.id, kind: candidate.id, createdAt: now.toISOString(), status: 'sent', to: family.guardian.email, subject, body, providerMessageId: provider.id || null }
  ...
  if (missedIntakeApplications.length) {
    ...
    if (template?.active) {
      ...
      const body = `${fill(template.body, { outstandingList })}\n\nOpen the staff dashboard: ${APP_URL}/#/staff\n\nRegards,\n1330 Squadron Staff`
      const provider = await sendEmail({ to: STAFF_ALERT_EMAIL, subject, body })
      for (const item of missedIntakeApplications) {
        const message = { id: `message-${crypto.randomUUID()}`, familyId: item.family.id, cadetId: item.cadet.id, templateId: template.id, alertKey: item.alertKey, kind: template.id, createdAt: now.toISOString(), status: 'sent', to: STAFF_ALERT_EMAIL, subject, body, providerMessageId: provider.id || null }
