...
function PaymentReturnPage({ preview = false }) {
  ...
  return <main className="mx-auto flex min-h-screen max-w-lg items-center px-5 py-12"><section className={`w-full border-2 bg-white p-8 text-center ${cancelled ? 'border-[var(--gold)]' : 'border-[var(--green)]'}`}><div className={`mx-auto flex h-14 w-14 items-center justify-center rounded-full text-2xl font-bold text-white ${cancelled ? 'bg-[var(--amber)]' : 'bg-[var(--green)]'}`}>{cancelled ? '!' : '✓'}</div><p className="mt-5 text-xs font-bold uppercase tracking-wide text-[var(--blue)]">{kind === 'subs' ? 'Direct Debit' : 'Joining fee'}</p><h1 className="mt-2 text-2xl font-semibold text-slate-900">{cancelled ? 'Setup not completed' : 'Bank step completed'}</h1><p className="mt-3 text-sm leading-6 text-slate-600">{cancelled ? 'Nothing has been confirmed. Return to the device where you started the joining form and try again when ready.' : 'Return to the device and browser tab where you started the joining form. It will confirm the result and continue automatically.'}</p>{originalTabIsOpen && <button type="button" onClick={() => window.close()} className="mt-6 rounded-lg bg-[var(--blue)] px-5 py-2.5 text-sm font-semibold text-white">Close this tab</button>}<p className="mt-5 text-xs text-slate-400">You can safely close this page. Do not start the joining form again on this device.</p></section></main>
}
