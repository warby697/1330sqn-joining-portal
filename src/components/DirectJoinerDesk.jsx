...
const empty = { guardianName: '', guardianEmail: '', cadetName: '' }
...
export default function DirectJoinerDesk({ onDataChanged }) {
  ...
  const submit = async (event) => {
    ...
    if (busy) return
    if (!values.guardianEmail.trim() || !values.cadetName.trim()) {
      setError('A parent email and the cadet name are both required.')
      return
    }
    setBusy(true)
    setError('')
    ...
    <section className="mt-6 border border-slate-200 bg-white p-6">
      ...
      <form onSubmit={submit} className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <Field label="Cadet full name" help="Date of birth, school year, mobile and address are collected from the parent in the Form 3822.">
