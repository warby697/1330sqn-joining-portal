...
import { resolveFormData, resolveResume, resolveStage, stageAfterFee, stageAfterForms, withFormPayments } from '../lib/paperworkResume'
...
export default function JoiningJourney({ familyId, cadetId, navigate, previewFamily = null, previewStage = '' }) {
  ...
  const [wizardIndex, setWizardIndex] = useState(resolveResume(saved, resumeCadet)?.wizardIndex || 0)
  ...
  const goNext = () => {
    ...
    // Straight past any payment already made, so a parent who has paid is never asked again.
    else setStage(stageAfterForms(withFormPayments(resumeCadet || cadet, formData)))
  }
